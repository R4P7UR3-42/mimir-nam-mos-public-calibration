...
class WorkflowTest(unittest.TestCase):
    def test_workflow_is_single_attempt_public_and_non_authorizing(self) -> None:
        text = (ROOT / ".github/workflows/nbm-tail-development-v2.yml").read_text(encoding="utf-8")
        self.assertIn("workflow_dispatch:", text)
        self.assertIn("github.run_attempt == 1", text)
        self.assertIn("runs-on: ubuntu-latest", text)
        self.assertIn("--max-requests 3000", text)
        self.assertIn("noaa_nbm_v5_station_robust_offered_tail_no_split_development_v2", text)
