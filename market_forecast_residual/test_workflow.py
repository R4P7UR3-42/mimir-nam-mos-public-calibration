...
class WorkflowTest(unittest.TestCase):
    def test_workflow_is_public_training_only_and_order_free(self):
        text = Path("../.github/workflows/market-forecast-residual-development.yml").read_text()
        self.assertIn("workflow_dispatch:", text)
        self.assertIn("runs-on: ubuntu-latest", text)
        self.assertIn("--max-requests 5000", text)
        self.assertIn("training_only:true", text)
        self.assertIn("evaluation_series_accessed:false", text)
